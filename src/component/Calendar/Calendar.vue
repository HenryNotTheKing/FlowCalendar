      ...
      <div class="calendar-header">
        <div class="month-switcher">
          <div class="arrow-container arrow-eft" @click="toPrevMonth">
            ...
            <img
              v-else
              src="../../assets/Icons/chevron-left-dark.svg"
              alt="arrow-left"
              class="icon"
            />
          </div>
          ...
          <div class="arrow-container arrow-right" @click="toNextMonth">
            ...
            <img
              v-else
              src="../../assets/Icons/chevron-right-dark.svg"
              alt="arrow-right"
              class="icon"
            />
          </div>
...
<script>
import { DateDisplay } from "../../stores/Calendar/DateDisplay.ts";
...
export default {
  ...
  methods: {
    ...
    toPrevMonth() {
      const prevDate = this.getPrevMonthDate(this.todayDate);
      this.todayDate = prevDate;
      if (this.todayDate.getDate() !== this.useDateDisplay.selectedDate?.getDate()) {
        const newDate = new Date(this.useDateDisplay.selectedDate);
        newDate.setMonth(prevDate.getMonth());
        newDate.setFullYear(prevDate.getFullYear());

        // 处理跨月日期溢出
        if (newDate.getMonth() !== prevDate.getMonth()) {
          newDate.setDate(0); // 设置为上月最后一天
        }
        this.useDateDisplay.selectedDate = newDate;
      }
      this.updateMonthDateList();
      this.checkTodayInCurrentMonth();
    },
    toNextMonth() {
      const nextDate = this.getNextMonthDate(this.todayDate);
      this.todayDate = nextDate;

      if (nextDate.getDate() !== this.useDateDisplay.selectedDate?.getDate()) {
        const newDate = new Date(this.useDateDisplay.selectedDate);
        newDate.setMonth(nextDate.getMonth());
        newDate.setFullYear(nextDate.getFullYear());

        if (newDate.getMonth() !== nextDate.getMonth() % 11) {
          newDate.setDate(1); // 处理跨年情况
        }
        this.useDateDisplay.selectedDate = newDate;
      }
    ...
    checkTodayInCurrentMonth() {
      ...
      if (
        ...
        !this.isSameMonth(this.useDateDisplay.selectedDate, today)
      ) {
        this.useDateDisplay.selectedDate = new Date(today);
      }
    },
    updateMonthDateList() {
...
/* 暗色主题样式 */
[data-theme="dark"] .chen-calendar {
  border-bottom: #484848 1px solid;
}

[data-theme="dark"] .calendar-date:hover::before {
  background: #3d3d3d;
}
...
[data-theme="dark"] .date-curr-month {
  color: #ffffff;
}

[data-theme="dark"] .date-prev-month,
[data-theme="dark"] .date-next-month {
  color: #666666;
}

[data-theme="dark"] .arrow-container:hover {
