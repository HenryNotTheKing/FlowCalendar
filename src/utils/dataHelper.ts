...
export function getRectPositionFromTimeRange(Event: ScheduleEvent): Rect {
...
export class RecurrenceService {
  ...
  static generateRecurrenceEvents(
    ...
    endDate: Date
      ...
      const queryEnd = originalEvent.recurrence.endCondition === 'untilDate' && originalEvent.recurrence.endDate
        ? new Date(Math.min(endDate.getTime(), this.ensureDate(originalEvent.recurrence.endDate).getTime()))
        : endDate;
      ...
      // 设置精确的时间范围（包含全天）
      const adjustedStart = dayjs(startDate).utc(true).startOf('day').toDate()
      const adjustedEnd = dayjs(queryEnd).utc(true).endOf('day').toDate()
    ...
    return dates
      .map(date => {
        // 保留原始时间部分
        const adjustedDate = dayjs(date).local().toDate();
  ...
  private static createRRule(event: ScheduleEvent): RRule {
    const options: Partial<Options> = {
        dtstart: dayjs(event.start).utc(true).toDate(),
    ...
    switch (event.recurrence.endCondition) {
      case 'untilDate':
        options.until = dayjs(event.recurrence.endDate!).utc(true).toDate()
        break
      case 'occurrences':
  ...
  static editOccurrence(
    ...
    // 更新原始事件的例外日期
    const exceptions = [...(originalEvent.exceptions || [])];
    if (editedEvent.lastState) {
      exceptions.push(editedEvent.lastState.start);
    } else {
      exceptions.push(editedEvent.start);
    }
    return {
      updatedOriginal: {
        ...originalEvent,
  ...
  // 确保日期对象是有效的Date类型
  private static ensureDate(date: any): Date {
...
export class TaskRecurrenceService {
  // 生成指定时间范围内的重复任务
  static generateRecurrenceTasks(
    originalTask: TodoTask,
    startDate: Date,
    endDate: Date
  ): TodoTask[] {
      // 添加时间范围限制
      const queryEnd = originalTask.recurrence && originalTask.recurrence.endCondition === 'untilDate' && originalTask.recurrence.endDate
        ? new Date(Math.min(endDate.getTime(), originalTask.recurrence.endDate.getTime()))
        : endDate;
  
      // 创建包含完整时间范围的规则集
      const ruleSet = new RRuleSet();
      const rrule = this.createRRule(originalTask);
      ruleSet.rrule(rrule);
    ...
    return dates
      .map(date => {
        ...
        const adjustedDate = dayjs(date).local().toDate();
        adjustedDate.setHours(
          originalTask.date.getHours(),
          originalTask.date.getMinutes(),
          originalTask.date.getSeconds()
        );
        return this.createTaskInstance(originalTask, adjustedDate);
      })
  }

    // 创建任务实例
  private static createTaskInstance(
    original: TodoTask,
    date: Date
  ): TodoTask {
    const newClone = {
  ...
  // 创建RRule实例
  private static createRRule(task: TodoTask): RRule {
    if (!task.recurrence) {
      throw new Error('Task has no recurrence rule');
    }
    
    const options: Partial<Options> = {
        dtstart: dayjs(task.date).utc(true).toDate(),
        freq: this.getFrequency(task.recurrence.type),
        interval: task.recurrence.interval,
    }

    // 处理星期规则
    if (task.recurrence.daysOfWeek) {
      options.byweekday = task.recurrence.daysOfWeek.map(day => 
          [RRule.MO, RRule.TU, RRule.WE, RRule.TH, RRule.FR, RRule.SA, RRule.SU][day - 1]
      )
  }

    switch (task.recurrence.endCondition) {
      case 'untilDate':
        options.until = dayjs(task.recurrence.endDate!).utc(true).toDate();
        break;
      case 'occurrences':
        options.count = task.recurrence.occurrences;
        break;
    }
